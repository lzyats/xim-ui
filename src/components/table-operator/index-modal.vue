      ...
      <template #bodyCell="{ text, record, index, column }">
        <template v-if="column.dataIndex === 'title'">
          <a-button type="text" :class="record.fixed ? '' : 'handle'" size="small"
            ...
            <template #icon>
              <template v-if="record.fixed"> <pushpin-outlined /> </template>
              <template v-else> <drag-outlined /> </template>
            </template>
            {{ text }}
          </a-button>
        </template>
...
async function getUserTableColumns(tableId, columns) {
  ...
  let res = await columnApi.getColumns(tableId);
  if (res.data) {
  ...
  Modal.confirm({
    ...
    onOk() {
      (async () => {
        ...
        try {
          await columnApi.deleteColumns(tableId);
...
async function save() {
  ...
  try {
    ...
    await columnApi.editColumn({
      tableId,
      content: JSON.stringify(columnList),
    });
