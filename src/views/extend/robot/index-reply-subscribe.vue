  ...
  <div>
    <a-row class="smart-table-btn-block">
      ...
      <div class="smart-table-setting-block">
        <TableOperator v-model="columns" :tableId="TABLE_ID.BL" :refresh="queryList" />
      </div>
    </a-row>
      ...
      <template #bodyCell="{ record, column, index }">
        ...
        <template v-if="column.dataIndex === 'action'">
          <div class="smart-table-operate">
            ...
            <a-button @click="copySubscribe(record.replyId)" type="link">复制</a-button>
...
import SubscribeModal from './index-reply-subscribe-edit.vue';
...
async function delSubscribeReply(id) {
  try {
    ...
    await robotApi.deleteSubscribeReply(id);
...
function copySubscribe(id) {
  Modal.confirm({
    ...
    content: '确认复制当前关注回复吗?',
    onOk() {
      copySubscribeReply(id);
    },
  });
}


async function copySubscribeReply(id) {
  try {
    tableLoading.value = true;
    await robotApi.copySubscribeReply(id);
...
async function queryList() {
  try {
    tableLoading.value = true;
    const queryResult = await robotApi.subscribeReply(queryForm);
