<template>
  <a-drawer title="签到详情" :width="600" :open="visible" :body-style="{ paddingBottom: '80px' }" @close="onClose">
    <a-card style="margin-bottom: 15px" size="small">
      ...
      <a-divider orientation="left">签到信息</a-divider>
      <a-descriptions :column="2" size="small">
        <a-descriptions-item label="签到金额">{{ chatUser.amount }}</a-descriptions-item>
        <a-descriptions-item label="签到日期">{{ chatUser.createTime }}</a-descriptions-item>
        <a-descriptions-item label="处理日期">{{ chatUser.updateTime }}</a-descriptions-item>
      </a-descriptions>
      <a-divider orientation="left">订单信息</a-divider>
      <a-descriptions :column="1" size="small">
        <a-descriptions-item label="签到ID">{{ chatUser.sourceId }}</a-descriptions-item>
        <a-descriptions-item label="交易单号">{{ chatUser.tradeNo }}</a-descriptions-item>
      </a-descriptions>
    </a-card>
    <div class="footer">
