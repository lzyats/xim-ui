  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    ...
    <a-row class="smart-table-btn-block">
      ...
      <div class="smart-table-setting-block">
        <TableOperator v-model="columns" :tableId="TABLE_ID.AN" :refresh="queryList" />
      </div>
    </a-row>
    <!---------- 表格操作行 end ----------->
    ...
    <a-table size="small" :dataSource="tableData" :columns="columns" rowKey="fileId" bordered :loading="tableLoading"
      ...
      <template #bodyCell="{ record, column, index }">
        ...
        <template v-if="column.dataIndex === 'action'">
          <div class="smart-table-operate" v-privilege="'approve:auth:edit'">
            <a-button @click="handView(record.userId)" type="link">
              认证详情
            </a-button>
          </div>
        </template>
...
function handView(userId) {
  approveRef.value.showModal(userId);
}
...
async function queryList() {
  ...
  try {
    let queryResult = await approveAuthApi.queryList(queryForm);
