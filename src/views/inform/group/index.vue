  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    ...
    <a-row class="smart-table-btn-block">
      ...
      <div class="smart-table-setting-block">
        <TableOperator v-model="columns" :tableId="TABLE_ID.AM" :refresh="queryList" />
      </div>
    </a-row>
    <!---------- 表格操作行 end ----------->
      ...
      <template #bodyCell="{ record, column, index }">
        ...
        <template v-if="column.dataIndex === 'targetNo'">
          ...
          <p v-privilege="'chat:group:query'">
            <a @click="viewGroup(record.targetId)">
              群组详情
            </a>
          </p>
        </template>
        ...
        <template v-if="column.dataIndex === 'action'">
          <div class="smart-table-operate" v-privilege="'inform:group:banned'">
            <a-button @click="handView(record)" v-if="record.status == 'N'" type="link">
              处理举报
            </a-button>
          </div>
        </template>
...
import { USER_PAGE_PATH, GROUP_PAGE_PATH } from '/@/constants/sys/home-const';
...
async function queryList() {
  ...
  try {
    let queryResult = await informGroupApi.queryList(queryForm);
